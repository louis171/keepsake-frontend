...
const MemoryEditFormModal = (props) => {
  const { user } = useContext(AuthContext);
  const { toast, setRefresh, memory } = props;
  ...
  const [memoryBody, setMemoryBody] = useState(memory.memoryBody);
  ...
  const handleSubmit = (e) => {
    ...
    if (form.checkValidity() === false) {
      ...
      axios
        ...
        .then((res) => {
          toast.success("Success! Added memory");
          setModalShow(false);
          setRefresh(true);
        })
        .catch((err) => {
...
export default MemoryEditFormModal;
